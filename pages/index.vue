  ...
  <div style="width: 100%; min-height: 100vh; display: inline-flex;">
    <div class="messages">
      <div v-for="(message, ind) in messages" :key="ind" :title="JSON.stringify(message)">
        ...
        <div class="msg" v-if="message.source === 'BOARD'">&#9992</div>
      </div>
    </div>
    <div class="radar">

      <div class="planeArea">
        <Plane v-for="(row, ind) in boards" :key="ind" :level="ind" :plane="row"/>
      </div>

      <div style="padding: 10px; text-align: center; colot: #fff; font-size: 200%">
...
export default {
  ...
  mounted() {
    this.defineSocket()
    setInterval(this.wakeSocketUp, 2000)
  },
  methods: {
    defineSocket() {
    ...
    addRoute(port) {
      ...
      if(this.tempRoute.length > 0) {
        last = this.tempRoute[this.tempRoute - 1]
      }
...
<style>
  *{
  ...
  .radar {
    width: 90%;
    position: relative;
    background: transparent url("assets/grass.jpg");
    min-height: 100vh;
  }
  .msg {
    border-radius: 10px;
